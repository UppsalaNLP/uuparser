...
class ArcHybridLSTM:
    ...
    def __evaluate(self, stack, buf, train):
        ...
        irels = self.feature_extractor.irels

        if train:
            ret = [ [ (rel, 0, scrs[2 + j * 2] + uscrs[2], routput[2 + j * 2 ] +
                       output[2]) for j, rel in enumerate(irels) ] if left_arc_conditions else [],
                   [ (rel, 1, scrs[3 + j * 2] + uscrs[3], routput[3 + j * 2 ] +
                      output[3]) for j, rel in enumerate(irels) ] if right_arc_conditions else [],
                   [ (None, 2, scrs[0] + uscrs[0], routput[0] + output[0]) ] if shift_conditions else [] ,
                    [ (None, 3, scrs[1] + uscrs[1], routput[1] + output[1]) ] if swap_conditions else [] ]
        else:
            ...
            ret = [ [ (r1, 0, s1) ] if left_arc_conditions else [],
                   ...
                   [ (None, 2, scrs[0] + uscrs[0]) ] if shift_conditions else [] ,
                    [ (None, 3, scrs[1] + uscrs[1]) ] if swap_conditions else [] ]
        return ret
