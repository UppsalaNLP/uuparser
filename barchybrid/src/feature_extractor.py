...
class FeatureExtractor(object):
    def __init__(self,model,words,rels,langs,w2i,ch,nnvecs,options):
        ...
        if self.disable_bilstm:
            self.lstm_output_size = int(0.5*lstm_input_size)
        ...
        if not self.disable_bilstm:
            self.surfaceBuilders = [dy.VanillaLSTMBuilder(1, lstm_input_size, self.lstm_output_size, model),
                                    dy.VanillaLSTMBuilder(1, lstm_input_size, self.lstm_output_size, model)]
            self.bsurfaceBuilders = [dy.VanillaLSTMBuilder(1, 2* self.lstm_output_size,
                                                           self.lstm_output_size , model),
                                     dy.VanillaLSTMBuilder(1, 2* self.lstm_output_size,
                                                       self.lstm_output_size , model)]


        self.charBuilders = [dy.VanillaLSTMBuilder(1, self.char_emb_size, self.char_lstm_output_size, model),
                             dy.VanillaLSTMBuilder(1, self.char_emb_size, self.char_lstm_output_size, model)]

        self.clookup = model.add_lookup_parameters((len(ch) + 1, self.char_emb_size))
        self.wlookup = model.add_lookup_parameters((len(words) + 2, self.word_emb_size))
